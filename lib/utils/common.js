            ...
            switch (i.type) {
                ...
                case 'image':
                    if (this.isLocalPath(i.file)) {
                        let file = fs.readFileSync(await this.getFilePath(i.file), 'base64url')
                        i.file = `base64://${file}`
                    }
                    if(isNode) {
                        ncmsg.push(Structs.image(i.file)) //合并转发内的图片不能传入summary、sub_type
                    } else {
                        ncmsg.push(Structs.image(i.file, i.summary || `[图片]`, i.asface ? 1 : 0))
                    }
                    raw_msg.push(`[图片]`)
                    break
                default:
            ...
            switch (i.type) {
                ...
                case 'at':
                    ...
                    try {
                        let qq = i.data.qq
                        ToString.push(`{at:${qq}}`)
                        let groupMemberList = Bot[botid].gml.get(group_id)?.[qq]
