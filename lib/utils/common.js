...
export default new class nccommon {
     ...
     * @param args
     */
    info(botInfo = { nickname: 'Napcat', uin: '未连接' }, ...args) {
     ...
     * @param args
     */
    mark(botInfo = { nickname: 'Napcat', uin: '未连接' }, ...args) {
     ...
     * @param args
     */
    warn(botInfo = { nickname: 'Napcat', uin: '未连接' }, ...args) {
     ...
     * @param args
     */
    error(botInfo = { nickname: 'Napcat', uin: '未连接' }, ...args) {
     ...
     * @param args
     */
    debug(botInfo = { nickname: 'Napcat', uin: '未连接' }, ...args) {
    ...
    async delTempFile() {
        try {
            const fileList = await fs_.readdir('./temp/')
            const tempFileList = fileList.filter(item => item.startsWith('napcat-file-temp'))
            Promise.all(tempFileList.map(item => fs_.unlink(path.join('./temp/', item))))
        } catch (error) { }
    }
    /**
     * 标准化文件消息
            ...
            switch (i.type) {
                ...
                case 'image':
                    ...
                    if(isNode) {
                        ...
                        let summary
                        if(i.asface && !i.summary) summary = `[动画表情]` 
                            else summary = i.summary || `[图片]`;
                        ncmsg.push(Structs.image(i.url ? i.url : i.file, summary, i.asface ? 1 : 0))
                    }
                    raw_msg.push(`[${i.asface ? '动画表情' : '图片'}]`)
                    break
                case 'json':
    ...
    /**
     * 判断是否为TRSS格式的合并转发消息
     * TRSS格式: { type: "node", data: [{ message, nickname, user_id }, ...] }
     * @param msg
     */
    isTRSSForwardMsg(msg) {
        if (!msg || typeof msg !== 'object') return false
        // TRSS格式的特征是 type 为 "node" 且 data 是数组
        if (msg.type === 'node' && Array.isArray(msg.data)) return true
        return false
    }

    /**
     * 将TRSS格式的合并转发消息转换为napcat-adapter支持的格式
     * TRSS格式: { type: "node", data: [{ message, nickname, user_id, time }, ...] }
     * NCAd格式: { test: true, message: [{ type: "node", data: { nickname, user_id, content, time } }, ...], data: {...} }
     * @param msg TRSS格式的转发消息
     * @param botNickname Bot昵称，用于默认昵称
     * @param botUin Bot UIN，用于默认用户ID
     */
    convertTRSSForwardMsg(msg, botNickname = 'Bot', botUin = '10000') {
        if (!this.isTRSSForwardMsg(msg)) return msg
        ...
        for (const item of msg.data) {
            ...
            const nodeData = {
                type: 'node',
                data: {
                    nickname: item.nickname || botNickname,
                    user_id: String(item.user_id || botUin),
            ...
            // 处理TRSS中可能存在的news字段
            if (item.news) {
                nodeData.data.news = item.news
            }

            forwardMsg.message.push(nodeData)
        }
    ...
    async getMessage(msg, group_id, reply = true, botid, napcat) {
        ...
        for (let i of msg) {
            switch (i.type) {
                /** AT 某人 */
                case 'at':
                    let uin = Bot.gl.get(group_id)?.uin || Bot.uin
    ...
    /**
     * 保存Bot数据
     * @param bc BotClient Bot实例
     * @param data 需保存的数据
     * @param ct cacheTime 缓存时间，默认不限
     * @param key 缓存键，默认使用数据的key
     */
    async saveBotData(bc, data, ct, key) {
        if(data instanceof Map) {
            // 转为数组后再序列化
            data = this.mapToJson(data);
        } else {
            // 尝试直接序列化
            try {
                data = JSON.stringify(data)
            } catch (error) {
                this.error(bc, '序列化数据失败，数据缓存失败', error)
            }
        }

        try {
            let rk = `cache:ncad-data:${bc.uin}:${key}`
            let res = await redis.set(rk, data, { EX: ct * 60 || '' })
        } catch (error) {
            this.error(this.bot, '缓存数据失败', error)
        }
        return true
    }
