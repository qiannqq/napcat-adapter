    ...
    getFile(file) {
        if(this.isLocalPath(file)) {
            try {
                ...
                if(rawFile.length > 10485760) return this.getFilePath(file)
    ...
    trackPropertyWithStack(obj, prop, contextName = 'Object') {
        ...
        Object.defineProperty(obj, prop, {
          ...
          set: (newVal) => {
            ...
            console.log(`[${contextName}] 属性 "${prop}" 被修改:\n  原值 → ${value}\n  新值 → ${newVal}`);
            console.log('调用轨迹:\n' + filteredStack);
        ...
        for (let i of icmsg) {
            switch (i.type) {
                ...
                case 'file':
                    ...
                    break
                case 'record':
                    ncmsg.push(Structs.record(this.getFile(i.file)))
                    raw_msg.push(`{record}`)
                    break
                case 'video':
                    ncmsg.push(Structs.video(this.getFile(i.file)))
                    raw_msg.push(`{video}`)
                    break
                case 'image':
