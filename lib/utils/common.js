        ...
        for (let i of icmsg) {
            switch (i.type) {
                ...
                case 'file':
                    ...
                    break
                case 'record':
                    ncmsg.push(Structs.record(this.isLocalPath(i.file) ? this.getFilePath(i.file) : i.file))
                    raw_msg.push(`{record}`)
                    break
                case 'video':
                    ncmsg.push(Structs.video(this.isLocalPath(i.file) ? this.getFilePath(i.file) : i.file))
                    raw_msg.push(`{video}`)
                    break
                case 'image':
    ...
    async getMessage(msg, group_id, reply = true, botid, napcat) {
        ...
        for (let i of msg) {
            switch (i.type) {
                ...
                /** 图片 */
                case 'image':
                    message.push({ ...i.data, type: 'image', md5: await this.getImageMD5(i.data.url) })
